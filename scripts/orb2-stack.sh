#!/usr/bin/env bash
# orb2 stack control — thin wrapper over docker compose for the
# single-user Spark stack. Keeps the compose file path in one place.
#
#   ./scripts/orb2-stack.sh up        # start the whole stack
#   ./scripts/orb2-stack.sh down      # stop + remove
#   ./scripts/orb2-stack.sh restart [svc]
#   ./scripts/orb2-stack.sh status    # ps + health
#   ./scripts/orb2-stack.sh logs [svc]
#   ./scripts/orb2-stack.sh heal      # tail the watchdog's healing log
set -euo pipefail
...
case "${1:-status}" in
  ...
  heal)     dc exec watchdog tail -f /var/log/orb2/watchdog.log ;;
