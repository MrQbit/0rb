...
# Requires: Python 3.10+, RAK00N_HF_TOKEN env var for gated model download.
set -euo pipefail

HF_TOKEN="${RAK00N_HF_TOKEN:-${HF_TOKEN:-}}"

echo "=== rak00n PersonaPlex (Moshi) Setup ==="
...
if [ -n "$HF_TOKEN" ]; then
  ...
  echo "⚠ RAK00N_HF_TOKEN not set — model download may fail for gated models"
fi
