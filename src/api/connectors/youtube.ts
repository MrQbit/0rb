 ...
 * Configure ORB2_YOUTUBE_API_KEY (Google Cloud → YouTube Data API v3 key).
 */
export function youtubeEnabled(): boolean {
  return !!(process.env.ORB2_YOUTUBE_API_KEY || '').trim()
}
...
export async function youtubeSearch(query: string, max = 8): Promise<YtResult[]> {
  const key = (process.env.ORB2_YOUTUBE_API_KEY || '').trim()
  if (!key) throw new Error('YouTube is not connected (ORB2_YOUTUBE_API_KEY unset)')
