 ...
 *
 * Off with RAK00N_MDNS=0. Best-effort — never blocks or crashes boot.
 */
...
export function advertiseOrb(port: number): void {
  if (service || process.env.RAK00N_MDNS === '0') return
  try {
    ...
    service = bonjour.publish({
      name: process.env.RAK00N_MDNS_NAME || '0rb',
    ...
    log.info('mdns_advertised', { service: '_0rb._tcp', name: process.env.RAK00N_MDNS_NAME || '0rb', port })
  } catch (err) {
