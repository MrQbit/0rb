...
describe('sandbox executor', () => {
  beforeEach(() => {
    delete process.env.ORB2_SANDBOX_ENABLED
    delete process.env.ORB2_SANDBOX_MODE
    delete process.env.ORB2_SANDBOX_URL
  })
  afterEach(() => {
  ...
  test('isSandboxEnabled is true by default and false when disabled', () => {
    ...
    process.env.ORB2_SANDBOX_ENABLED = '0'
    expect(isSandboxEnabled()).toBe(false)
    process.env.ORB2_SANDBOX_ENABLED = 'false'
    expect(isSandboxEnabled()).toBe(false)
  })
  ...
  test('returns disabled message when ORB2_SANDBOX_ENABLED=0', async () => {
    process.env.ORB2_SANDBOX_ENABLED = '0'
  ...
  test('inproc mode: child cannot see credential env vars', async () => {
    process.env.ORB2_SANDBOX_MODE = 'inproc'
    process.env.LLM_KEY = 'super-secret-leaked'
    process.env.OPENAI_API_KEY = 'sk-leaked'
    process.env.ORB2_BOOTSTRAP_ADMIN_KEY = 'orb2_leaked'
    ...
    expect(r.stdout).not.toContain('ORB2_BOOTSTRAP_ADMIN_KEY')
    expect(r.stdout).toContain('PATH')
    expect(r.stdout).toContain('HOME')
  })

  test('pod mode: dispatches HTTP POST to the sandbox URL', async () => {
    process.env.ORB2_SANDBOX_MODE = 'pod'
    process.env.ORB2_SANDBOX_URL = 'http://localhost:1' // unreachable
    const r = await executeCode('python3', 'print(1)', undefined)
    expect(r.exitCode).toBe(1)
