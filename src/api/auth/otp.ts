/**
 * Email + one-time-code authentication for orb2.
 *
 ...
 *   ORB2_AUTH_ALLOWED_EMAILS   comma-separated bootstrap allowlist
 *
 * Email delivery uses SMTP via `curl` (no new dependency); configure:
 *   ORB2_SMTP_HOST  ORB2_SMTP_PORT(=465)  ORB2_SMTP_USER  ORB2_SMTP_PASS
 *   ORB2_SMTP_FROM (defaults to USER)
...
// Same kv key the Settings UI writes (PUT /v1/settings ORB2_AUTH_ALLOWED_EMAILS),
// so the allowlist is editable from the console and takes effect immediately.
const ALLOWLIST_KEY = 'setting:ORB2_AUTH_ALLOWED_EMAILS'
...
function envAllowed(): string[] {
  return (process.env.ORB2_AUTH_ALLOWED_EMAILS || '')
 ...
 * (ORB2_TELEGRAM_OWNER_ID) so Telegram-delivered codes work out of the box.
 */
...
export async function availableChannels(store: Store, email: string): Promise<Array<'email' | 'telegram'>> {
  ...
  if (u?.telegram_chat_id && process.env.ORB2_TELEGRAM_BOT_TOKEN) out.push('telegram')
  return out
}

function codeHash(email: string, code: string): string {
  return createHash('sha256')
    .update(`${email}:${code}:${process.env.ORB2_AUTH_SECRET || ''}`)
    .digest('hex')
}
...
async function sendCodeTelegram(chatId: string, code: string): Promise<boolean> {
  ...
  try {
    const res = await fetch(`https://api.telegram.org/bot${token}/sendMessage`, {
      ...
      body: JSON.stringify({ chat_id: chatId, text: `🔐 Your orb2 sign-in code is ${code}\n\nIt expires in 10 minutes. If you didn't request it, ignore this message.` }),
    })
...
async function sendCodeEmail(to: string, code: string): Promise<boolean> {
  const host = process.env.ORB2_SMTP_HOST
  const user = process.env.ORB2_SMTP_USER
  const pass = process.env.ORB2_SMTP_PASS
  if (!host || !user || !pass) return false
  const port = process.env.ORB2_SMTP_PORT || '465'
  const from = process.env.ORB2_SMTP_FROM || user

  const message =
    `From: orb2 <${from}>\r\n` +
    `To: ${to}\r\n` +
    `Subject: Your orb2 sign-in code: ${code}\r\n` +
    `Content-Type: text/plain; charset=utf-8\r\n\r\n` +
    `Your orb2 sign-in code is ${code}\r\n\r\nIt expires in 10 minutes. If you didn't request it, ignore this email.\r\n`
