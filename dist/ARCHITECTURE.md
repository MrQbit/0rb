...
rak00n runs as a single **Docker Compose** stack on one machine (designed for a
...
```
                ┌──────────────────────── host (GB10) ─────────────────────────┐
 browser / iOS ─►│ ui (nginx :9080/:9443) ─► rak00n-api :8080 ─► vllm :8888 (brain)│
 Telegram      ─►│      │                       │  │                             │
...
- **rak00n-api** — the Bun agent: chat, tools, voice WS, channels, canvas/widgets,
...
The brain is configured purely via OpenAI-compatible env on `rak00n-api`:
...
with `scripts/rak00n-stack.sh {up|down|status|logs|heal}`.

## Self-evolution

The agent can edit its own source, build it, validate it in a throwaway sandbox
container, and promote it to the running `rak00n-api` with automatic rollback —
gated by `RAK00N_SELF_MODIFY_ENABLED` and the mounted repo + docker socket.
